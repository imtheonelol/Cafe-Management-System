  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl max-w-lg w-full max-h-[90vh] overflow-auto">
        <div className="p-6 border-b flex items-center justify-between sticky top-0 bg-white z-10"><h2 className="text-2xl font-bold">Checkout</h2><button onClick={onClose} className="text-gray-500"><X size={24}/></button></div>
        <div className="p-6">
          <div className="mb-6 bg-gray-50 rounded-lg p-4 border space-y-2">
            ...
            <div className="pt-2 border-t flex items-center justify-between">
              ...
              <select value={discountType} onChange={(e) => setDiscountType(e.target.value)} className="border rounded p-1 text-sm"><option value="none">None</option><option value="senior">Senior / PWD (20% + VAT Exempt)</option></select>
            </div>
          ...
          {discountType !== 'none' && <div className="mb-6"><label className="block text-sm font-semibold text-red-600 mb-2">Admin PIN Required</label><input type="password" value={adminPin} onChange={(e) => setAdminPin(e.target.value)} className="w-full px-4 py-2 border border-red-300 rounded-lg focus:ring-red-600 focus:ring-2" placeholder="****" /></div>}
          <div className="mb-6 space-y-2">
            <h3 className="font-semibold mb-3">Payment Method</h3>
            <button onClick={() => setPaymentMethod('cash')} className={`w-full p-4 rounded-lg border-2 flex items-center gap-3 ${paymentMethod === 'cash' ? 'border-blue-600 bg-blue-50' : 'border-gray-200'}`}><Wallet className="text-blue-600" /><div className="font-semibold">Cash</div></button>
            <button onClick={() => setPaymentMethod('card')} className={`w-full p-4 rounded-lg border-2 flex items-center gap-3 ${paymentMethod === 'card' ? 'border-blue-600 bg-blue-50' : 'border-gray-200'}`}><CreditCard className="text-blue-600" /><div className="font-semibold">Card</div></button>
          </div>
          {paymentMethod === 'cash' && (
          ...
          <div className="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200 flex items-center gap-3 cursor-pointer" onClick={() => setPrintReceipt(!printReceipt)}>
            <input type="checkbox" checked={printReceipt} readOnly className="w-5 h-5 accent-blue-600" /><div className="flex items-center gap-2 font-semibold text-blue-900"><Printer size={18}/> Print Customer Receipt</div>
          </div>
          <button onClick={handlePayment} disabled={isPayDisabled} className="w-full py-3 bg-green-600 text-white rounded-lg font-bold hover:bg-green-700 disabled:bg-gray-400">
