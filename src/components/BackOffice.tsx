...
export function BackOffice({ isOpen, onClose, products, categories, onAddProduct, onUpdateProduct, onDeleteProduct }: BackOfficeProps) {
  ...
  const [formData, setFormData] = useState({
    name: '', description: '', price: '', category_id: '', image_url: '', stock: '100', is_available: true,
  });

  if (!isOpen) return null;

  const resetForm = () => {
    setFormData({ name: '', description: '', price: '', category_id: '', image_url: '', stock: '100', is_available: true });
  ...
  const handleEdit = (product: Product) => {
    ...
    setFormData({
      ...
      category_id: product.category_id, image_url: product.image_url, stock: product.stock.toString(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const productData = {
      ...
      category_id: formData.category_id, image_url: formData.image_url, stock: parseInt(formData.stock),
      is_available: formData.is_available,
    };

    if (editingProduct) await onUpdateProduct(editingProduct.id, productData);
                    ...
                    {products.map((product) => (
                        <tr key={product.id} className="border-b border-gray-100">
                          ...
                          <td className="py-3 px-4 text-right">
                            ...
                            <button onClick={() => onDeleteProduct(product.id)} className="p-2 text-red-600 hover:bg-red-50 rounded"><Trash2 className="w-4 h-4" /></button>
                          </td>
                        </tr>
                    ))}
                  </tbody>
            ...
            <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-4">
              ...
              <div className="border p-4 rounded-lg bg-gray-50">
                ...
                <div className="flex gap-2 mb-3">
                  ...
                  <button type="button" onClick={() => setUploadMode('local')} className={`px-4 py-2 rounded-md flex items-center gap-2 text-sm ${uploadMode === 'local' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}><UploadCloud size={16}/> Upload Local File</button>
                </div>
                
                {uploadMode === 'url' ? (
                  <input type="url" value={formData.image_url} onChange={(e) => setFormData({ ...formData, image_url: e.target.value })} placeholder="https://images.example.com/..." className="w-full px-4 py-2 border rounded-lg" />
                ) : (
                  <input type="file" accept="image/*" onChange={handleLocalImageUpload} disabled={uploadingImage} className="w-full file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
                )}
                {uploadingImage && <p className="text-sm text-blue-600 mt-2">Uploading to cloud...</p>}
              </div>
