            ...
            activeTab === 'ingredients' ? (
              ...
              <div className="max-w-lg">
                ...
                <div className="bg-gray-50 p-6 border rounded-xl">
                  ...
                  <button onClick={async () => { await ApiService.updateSetting('business_day_start', bizStartTime); alert("Settings Saved!"); }} className="w-full py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700">Save Configuration</button>
                </div>
              </div>
            ) : activeTab === 'products' ? (
              <>
                <div className="flex justify-between items-center mb-6"><h3 className="text-lg font-semibold">Inventory</h3><button onClick={() => { setEditingProduct(null); setProductData({ name: '', description: '', price: '', category_id: '', image_url: '', stock: '100', is_available: true }); setShowForm(true); }} className="px-4 py-2 bg-blue-600 text-white rounded-lg flex items-center gap-2"><Plus size={16}/> Add</button></div>
                <table className="w-full text-left border">
