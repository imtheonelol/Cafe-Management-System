...
export function EndShiftModal({ isOpen, onClose, shift, employeeId, onConfirm }: any) {
  ...
  // ✨ The amount they give the manager (leaving the float behind)
  const amountToTurnOver = actual - startingFloat; 
    ...
    return (
      <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50 p-4 print:p-0 print:bg-transparent">
        <div className="bg-white rounded-xl max-w-md w-full p-6 print:shadow-none print:w-full print:max-w-none text-center">
          ...
          <div className="text-left space-y-1 mb-4 text-sm border-b pb-4">
            <h3 className="font-bold mb-1">Sales Summary</h3>
          ...
          <div className="text-left space-y-1 mb-4 text-sm">
            <h3 className="font-bold mb-1">Drawer Audit</h3>
            <div className="flex justify-between text-gray-500"><span>Starting Float:</span> <span>₱{startingFloat.toFixed(2)}</span></div>
          ...
          <div className="bg-gray-100 p-3 mt-4 rounded border font-bold text-lg flex justify-between print:bg-transparent print:border-t print:border-b print:rounded-none">
             <span>Cash to Remit:</span> <span>₱{amountToTurnOver.toFixed(2)}</span>
          </div>
          <div className="print:hidden mt-8 border-t border-gray-200 pt-4">
            <button onClick={onConfirm} className="w-full py-3 bg-red-600 text-white rounded-lg font-bold hover:bg-red-700">Close & Logout</button>
          </div>
        </div>
      </div>
    );
      ...
      <div className="bg-white rounded-xl max-w-md w-full p-6 relative">
        ...
        <form onSubmit={handleEndShift}>
          ...
          <div className="relative rounded-md shadow-sm mb-4">
            ...
            <input type="number" step="0.01" required value={actualCash} onChange={(e) => setActualCash(e.target.value)} className="focus:ring-red-500 w-full pl-10 text-3xl font-bold border rounded-md py-4 text-center" placeholder="0.00" />
          </div>
          {actualCash && (
            <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg mb-6 text-center">
              <p className="text-sm font-semibold text-blue-800 mb-1">Cash to Turn Over / Remit</p>
              <p className="text-2xl font-bold text-blue-900">₱{amountToTurnOver.toFixed(2)}</p>
              <p className="text-xs text-blue-700 mt-1">Leave ₱{startingFloat.toFixed(2)} in drawer.</p>
            </div>
          )}
