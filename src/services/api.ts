...
async function getDB() {
  ...
  if (!res.ok) throw new Error("Failed to fetch local database.");
...
export const ApiService = {
  ...
  // --- Shift Services ---
  async getActiveShift(employeeId: string) { const db = await getDB(); return db.shifts.find((s: any) => s.employee_id === employeeId && !s.end_time) || null; },
  async getLastShift() {
    const db = await getDB();
  ...
  async getCashSalesForShift(employeeId: string, startTime: string) {
    const db = await getDB();
    return db.orders.filter((o: any) => o.employee_id === employeeId && o.payment_method === 'cash' && new Date(o.created_at) >= new Date(startTime)).reduce((sum: number, order: any) => sum + order.total, 0);
  },
