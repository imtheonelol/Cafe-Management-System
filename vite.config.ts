...
import { Client, Pool } from 'pg'; // <-- Added Pool!
...
// Using 127.0.0.1 fixes the Windows IPv6 localhost issue
...
function postgresDatabasePlugin() {
  // We upgraded this from a single Client to a concurrent Pool
  let pool: Pool;
  let dbConnectionError: string | null = null;

  const initDB = async () => {
    try {
      // 1. Use a temporary Client just to create the database if it doesn't exist
      ...
      // 2. Connect to our App Database using a POOL to handle concurrent React requests!
      pool = new Pool({ 
      ...
      if (parseInt(userCount.rows[0].count) === 0) {
        await pool.query(`INSERT INTO profiles (id, email, password, full_name, role, created_at) VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP)`, ['1', 'admin@cafe.com', 'password', 'Admin Boss', 'admin']);
        await pool.query(`INSERT INTO profiles (id, email, password, full_name, role, created_at) VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP)`, ['2', 'staff@cafe.com', 'password', 'Friendly Barista', 'employee']);
